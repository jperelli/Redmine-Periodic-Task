...
- Add an end condition to a task: `Ends` on a date and/or after N scheduled runs; whichever comes first ends the task and the reason is recorded in the activity log. `Run now` does not count. The task lists and detail page show `Ends on <date>` / `<n> of <max> runs` next to the schedule **requires migration** (@jperelli)
- The `Active` flag becomes a `State`: `Active`, `Inactive` (paused by hand) or `Ended` (set by the scheduler when the end condition is reached, shown as `Ended on <date>`). In the task lists inactive tasks are greyed out instead of carrying a lock icon and ended tasks are greyed out and struck through like closed issues; the REST API exposes `state` and `ended_at` instead of `is_active` **requires migration** (@jperelli)
