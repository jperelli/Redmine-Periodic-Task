...
- Add an end condition to a task: `Ends` on a date and/or after N scheduled runs; whichever comes first ends the task and the reason is recorded in the activity log. Ended is computed from those two fields, independently of the `Active` flag (move the end date or raise N to let the task run again); `Run now` does not count. The task lists and detail page show `Ends on <date>` / `<n> of <max> runs` next to the schedule and `Ended (end date reached)` / `Ended (maximum number of runs reached)` next to the `Active` flag; in the lists inactive tasks are greyed out instead of carrying a lock icon and ended tasks are greyed out and struck through like closed issues. The REST API exposes `end_date`, `max_occurrences`, `occurrences_count`, `ended` and `end_reason` **requires migration** (@jperelli)
- Add an `Assignee rotation`: an ordered list of project members the generated issues are assigned to in turn (duty roster), skipping users who are no longer assignable and falling back to the single assignee; the form, detail page and lists show who is next, `Run now` rotates too and `Copy` restarts the rotation **requires migration** (@jperelli)
...
- Add a per-task `Previous issue open` setting: create a new issue anyway (default), skip the occurrence, close the previous issue first, or wait and restart the schedule from its closing date (Todoist's `every!`). Skipped and waiting runs are shown on the task detail page and as notes in the scheduler log, not as errors. Each mode is explained with examples in [doc/if-previous-open.md](doc/if-previous-open.md), linked from a help icon in the form; the `How schedules are calculated` link in the page footer is gone (the help icons next to the interval and the last error remain) **requires migration** (@jperelli)
...
- Find the generated issues from the issue list: a `Periodic task` filter (*any* / *none* / *is* a task of the project and its subprojects, all tasks on the global list; also for saved queries and `GET /issues.json?periodictask=<id>`), an optional sortable/groupable `Periodic task` column linking to the task, a recurrence marker next to the subject of generated issues and an *All issues generated by this task* link on the task page (@jperelli)
- Preview of the next occurrences: the task form shows the rule as a sentence, the next 5 run days as chips and a collapsible mini calendar, all refreshed live while the recurrence is edited (`Periodictask#upcoming_run_dates`, `preview` action); the task page shows the same chips and calendar in its schedule header, so rules like *last Friday of every month* can be checked before saving. The preview stops at the end condition (no run after `Ends on`, no more than the runs left of `After N runs`) and shows the reason instead when the task is ended (@jperelli)
...
- A run holds a database row lock on the task while it generates the issue and saves the task, and a scheduled run re-checks that the task is still due once it holds the lock: triggers firing at the same time (cron, the web scheduler, the endpoint, `Run now`) could create two issues for one occurrence or give one rotation turn to two issues (@jperelli)
