...
- Add an `Assignee rotation`: an ordered list of project members the generated issues are assigned to in turn (duty roster), skipping users who are no longer assignable and falling back to the single assignee; the form, detail page and lists show who is next, `Run now` rotates too and `Copy` restarts the rotation; runs hold a row lock on the task so concurrent triggers cannot give the same turn to two issues **requires migration** (@jperelli)
