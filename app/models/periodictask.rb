...
class Periodictask < (defined?(ApplicationRecord) ? ApplicationRecord : ActiveRecord::Base)
  ...
  belongs_to :issue_category, class_name: 'IssueCategory', foreign_key: 'issue_category_id', optional: true
  ...
  def self.normalize_selection(value, allowed)
    normalize_integers(value).select { |v| allowed.include?(v) }.sort
  end
  ...
  def self.normalize_user_ids(value)
    normalize_integers(value).select(&:positive?)
  end

  # Distinct integers out of form / API input (strings, blanks, duplicates).
  def self.normalize_integers(value)
    Array(value).filter_map { |v| Integer(v.to_s, 10, exception: false) }.uniq
  end
  ...
  def rotation_candidates
    self.class.rotation_candidates(project)
  end

  def self.rotation_candidates(project)
  ...
  DATE_MACRO = /\*\*(DAY|WEEKISO_YEAR|WEEKISO|WEEK|QUARTER|MONTHNAME|MONTH|YEAR)([+-]\d{1,4})?\*\*/

  # Macros for the week or month before/after the run, as [shift, component].
  # Each one has a year companion because the year of the shifted instant is
  # not always the year of the run (**PREVIOUS_MONTH** in January), and ISO
  # weeks belong to the ISO week-based year (%G), which differs from the
  ...
  # Time of the last issue generated by each of +tasks+, keyed by task id
  # (the "Last run" column of the task lists and the API).
  def self.last_run_dates(tasks)
  ...
  def fill_watchers(issue)
    ...
    User.where(id: watcher_user_ids).find_each { |user| issue.add_watcher(user) }
  end
  ...
  # Tuesday 10:00). Without a next_run_date to take the time of day from,
  # +closed_at+ itself is the anchor.
  def next_run_date_after_completion(closed_at)
    return next_occurrence(closed_at, closed_at.end_of_day) if next_run_date.nil?
  ...
  def parse_macro(str, now, current = nil)
    if str.respond_to?(:gsub!) && str.present?
      ...
      str.gsub!(SHIFTED_MACRO) do
        shift, component = SHIFTED_MACROS.fetch(Regexp.last_match(1))
        date_macro_value(component, now + shift)
      end
  ...
  def date_macro_value(name, time)
    ...
    when 'WEEKISO_YEAR' then time.strftime('%G')
